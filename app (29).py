...
def _traduzir_texto(texto):
    """
    Traduz um único texto de inglês para português via endpoint público do Google Translate.
    Sem chave de API. Retorna o texto original em caso de falha.
    """
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            # Estrutura: [ [ ["tradução", "original", ...], ... ], ... ]
            partes = data[0] if data and isinstance(data[0], list) else []
            traducao = "".join(p[0] for p in partes if p and p[0])
...
def _traduzir_lista(textos):
    """Traduz lista de strings, item a item. Retorna lista de mesmo tamanho."""
    return [_traduzir_texto(t) for t in textos]
...
def buscar_noticias_com_traducao(ticker_us):
    ...
    titulos_trad = _traduzir_lista(titulos_orig)
    ...
    if idx_desc:
        ...
        descs_trad = _traduzir_lista(list(descs))
...
def buscar_dados_fundamentalistas(ticker_bdr):
        ...
        if nome_limpo:
            try:
                ...
                for q in quotes:
                    ...
                    symbol = q.get('symbol', '')
                    # Aceita ações e ADRs em bolsas americanas (incluindo OTC para ADRs)
                    if tipo in ('EQUITY',) and '.' not in symbol and exchange in (
                        'NMS', 'NYQ', 'NGM', 'NCM', 'ASE', 'PCX', 'BTS', 'NAS',
                        'NYSE', 'NASDAQ', 'OTC', 'PNK', 'OTCBB', 'OPK'
                    ):
    ...
    # ------------------------------------------------------------------
    # TENTATIVA 2b: Yahoo Finance — variantes do ticker US
    # Cobre ADRs com sufixos alternativos (ex: MUFG, TM, SONY etc.)
    # ------------------------------------------------------------------
    try:
        # Monta lista de variantes a tentar
        variantes = set()
        base = ticker_us.upper()
        variantes.add(base)
        # Remove sufixos de classe comuns no mapa
        for sufixo in ['-B', '-A', '-C', '.B', '.A']:
            if base.endswith(sufixo):
                variantes.add(base[: -len(sufixo)])
        # Tenta o nome curto da empresa (primeiras palavras)
        nome_empresa = NOMES_BDRS.get(ticker_bdr, '')
        if nome_empresa:
            # Ex: "Mitsubishi UFJ Financial Group, Inc." → tenta "MUFG" já está no mapa
            # mas tenta também a busca pelo ticker mapeado com yf.Search ampliado
            try:
                resultado_busca = yf.Search(nome_empresa.split(',')[0], max_results=10)
                quotes = resultado_busca.quotes if hasattr(resultado_busca, 'quotes') else []
                for q in quotes:
                    symbol = q.get('symbol', '')
                    tipo   = q.get('quoteType', '')
                    # Aceita qualquer bolsa americana, incluindo OTC (para ADRs)
                    if tipo == 'EQUITY' and '.' not in symbol:
                        variantes.add(symbol)
            except Exception:
                pass

        for t in list(variantes)[:6]:
            if t == ticker_us:
                continue  # já foi tentado acima
            try:
                info = yf.Ticker(t).info
                resultado = _score_from_yf_info(info, f'Yahoo Finance — {t}', t)
                if resultado:
