...
def obter_dados_timeframe(ticker, timeframe):
    """Baixa dados do Yahoo Finance conforme o timeframe escolhido no gráfico principal."""
    configs = {
        "60 minutos": {"period": "60d", "interval": "60m"},
        "Diário": {"period": "1y", "interval": "1d"},
        "Semanal": {"period": "5y", "interval": "1wk"},
        "Mensal": {"period": "10y", "interval": "1mo"},
    }
    cfg = configs.get(timeframe, configs["Diário"])
    ticker_yf = f"{ticker}.SA" if not str(ticker).endswith(".SA") else ticker
    ...
    try:
        ...
        # Recalcula indicadores para o timeframe selecionado sem alterar o restante do app.
        ...
        df["BB_Upper"] = media20 + (2 * desvio20)
        df["BB_Lower"] = media20 - (2 * desvio20)
...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val, timeframe="Diário"):
    """Gráfico principal interativo com zoom, pan e timeframes."""

    df_origem = obter_dados_timeframe(ticker, timeframe)
    if df_origem is None or df_origem.empty:
        df_origem = df_ticker.copy()

    colunas_necessarias = ["Close", "Open", "High", "Low", "Volume", "EMA20", "RSI14", "Stoch_K", "BB_Lower", "BB_Upper"]
    colunas_opcionais = ["EMA50", "EMA200", "MACD_Hist"]
    colunas_presentes = [c for c in colunas_necessarias + colunas_opcionais if c in df_origem.columns]
    df = df_origem[colunas_presentes].copy()
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            f"{ticker} - {empresa} | I.S.: {is_val:.0f} | {status} | Timeframe: {timeframe} | Próx. Fib: {nivel_mais_proximo}",
            "RSI14",
            "Estocástico %K",
        ),
    )

    # Candlestick quando houver OHLC; linha de fechamento como fallback.
    if all(c in df.columns for c in ["Open", "High", "Low", "Close"]):
        ...
        fig.add_trace(
            go.Scatter(x=datas, y=close, name="Close", mode="lines", line=dict(color="#1a1a2e", width=2.4)),
            row=1,
            col=1,
        )
    ...
    if "BB_Lower" in df.columns and "BB_Upper" in df.columns:
        fig.add_trace(
            go.Scatter(x=datas, y=df["BB_Upper"], name="BB Superior", mode="lines", line=dict(color="#94a3b8", width=1), opacity=0.35),
            row=1,
            col=1,
        )
        fig.add_trace(
    ...
    fig.add_trace(go.Scatter(x=datas, y=ema20, name="EMA20", mode="lines", line=dict(color="#2962FF", width=1.8)), row=1, col=1)
    if ema50 is not None:
        fig.add_trace(go.Scatter(x=datas, y=ema50.reindex(datas), name="EMA50", mode="lines", line=dict(color="#FF6D00", width=1.7)), row=1, col=1)
    if ema200 is not None:
        fig.add_trace(go.Scatter(x=datas, y=ema200.reindex(datas), name="EMA200", mode="lines", line=dict(color="#00695C", width=2.0)), row=1, col=1)

    for nivel, preco_fib in fib_levels.items():
        fig.add_hline(y=preco_fib, line_dash="dash", line_width=1, opacity=0.35, row=1, col=1)
        fig.add_annotation(x=datas[-1], y=preco_fib, text=f"Fib {nivel}", showarrow=False, xanchor="left", font=dict(size=10), row=1, col=1)

    # RSI
    if "RSI14" in df.columns:
        fig.add_trace(go.Scatter(x=datas, y=df["RSI14"].reindex(datas), name="RSI14", mode="lines", line=dict(color="#FF6F00", width=1.7)), row=2, col=1)
        fig.add_hline(y=30, line_dash="dash", line_color="#F44336", opacity=0.65, row=2, col=1)
        fig.add_hline(y=70, line_dash="dash", line_color="#4CAF50", opacity=0.65, row=2, col=1)

    # Estocástico
    if "Stoch_K" in df.columns:
        fig.add_trace(go.Scatter(x=datas, y=df["Stoch_K"].reindex(datas), name="Stoch %K", mode="lines", line=dict(color="#9C27B0", width=1.7)), row=3, col=1)
        fig.add_hline(y=20, line_dash="dash", line_color="#F44336", opacity=0.65, row=3, col=1)
        fig.add_hline(y=80, line_dash="dash", line_color="#4CAF50", opacity=0.65, row=3, col=1)
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col1:
                    timeframe_grafico = st.radio(
                        "Timeframe do gráfico",
                        ["60 minutos", "Diário", "Semanal", "Mensal"],
                        index=1,
                        horizontal=True,
                        key=f"tf_grafico_{ticker}",
                    )
                    fig = plotar_grafico(df_ticker, ticker, row['Empresa'], row['RSI14'], row['IS'], timeframe=timeframe_grafico)
                    st.plotly_chart(
                        fig,
                        use_container_width=True,
                        config={
