...
def analisar_triple_screen(df_ticker):
    ...
    try:
        ...
        return {
            ...
            # Séries para os mini-gráficos (últimos 60 dias)
            'serie_close':    close.iloc[-60:],
            'serie_macd':     macd_elder.iloc[-60:],
    ...
    with st.expander("🖥️ Estratégia Triple Screen — Alexander Elder", expanded=False):
        ...
        for col, tela, num, nome, subtitulo in [
            ...
            with col:
                ...
                if num == "1ª":
                    # MACD Elder — linha + zona positiva/negativa
                    xs = range(len(serie_macd))
                    vals = serie_macd.values
                    ax_m.plot(xs, vals, color=brd_s, linewidth=1.5, zorder=3)
                    ax_m.fill_between(xs, vals, 0,
                                      where=(vals >= 0), alpha=0.25,
                                      color='#43a047', interpolate=True)
                    ax_m.fill_between(xs, vals, 0,
                                      where=(vals < 0),  alpha=0.25,
                                      color='#e53935', interpolate=True)
                    ax_m.axhline(0, color='#90a4ae', linewidth=0.8, linestyle='--')
                    ax_m.set_title("MACD Elder", fontsize=7, color=txt_s, pad=3)
