...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val):
    ...
    # Bollinger Bands — área suave + bordas coloridas nítidas
    if 'BB_Lower' in df.columns and 'BB_Upper' in df.columns:
        bb_lower_s = df['BB_Lower'].reindex(datas)
        bb_upper_s = df['BB_Upper'].reindex(datas)
        # Área interna levemente preenchida
        ax1.fill_between(datas, bb_lower_s, bb_upper_s,
                         alpha=0.09, color='#5c6bc0', zorder=0)
        # Borda superior (sobrecompra) — vermelho
        ax1.plot(datas, bb_upper_s, color='#e53935', linewidth=1.2,
                 linestyle='--', alpha=0.80, zorder=2, label='BB Sup.')
        # Borda inferior (sobrevenda) — verde
        ax1.plot(datas, bb_lower_s, color='#43a047', linewidth=1.2,
                 linestyle='--', alpha=0.80, zorder=2, label='BB Inf.')
            ...
            try:
                ...
                with tf_col1:
                    timeframe = st.radio(
                        ...
                        key=f"tf_{ticker}",
                        help=(
                            "Diário: cada candle = 1 dia de negociação.\n"
                            "Semanal: cada candle = 1 semana (OHLCV agregado). "
                            "Útil para identificar a tendência maior (1ª Tela do Triple Screen)."
                        )
                    )

                # Reamostrar para semanal se solicitado
                if timeframe == "Semanal":
                    # Agrega OHLCV por semana (fecha na sexta)
                    regras = {
                        'Open':   'first',
                        'High':   'max',
                        'Low':    'min',
                    ...
                    # Recalcula indicadores no timeframe semanal
                    colunas_ohlcv = [c for c in ['Open','High','Low','Close','Volume']
                                     if c in df_ticker.columns]
                    df_semanal = (df_ticker[colunas_ohlcv]
                                  .resample('W-FRI').agg(
                                      {c: regras[c] for c in colunas_ohlcv}
                                  ).dropna())

                    # Recalcula indicadores derivados no timeframe semanal
                    close_w  = df_semanal['Close']
                    high_w   = df_semanal['High']
                    low_w    = df_semanal['Low']
                    volume_w = df_semanal['Volume']

                    # EMAs
                    df_semanal['EMA20']  = close_w.ewm(span=20, adjust=False).mean()
                    df_semanal['EMA50']  = close_w.ewm(span=50, adjust=False).mean()
                    df_semanal['EMA200'] = close_w.ewm(span=200, adjust=False).mean()
                    ...
                    perda  = -delta.clip(upper=0).rolling(14).mean()
                    rs     = ganho / perda
                    df_semanal['RSI14'] = 100 - (100 / (1 + rs))

                    # Estocástico 14
                    ll = low_w.rolling(14).min()
                    hh = high_w.rolling(14).max()
                    df_semanal['Stoch_K'] = 100 * ((close_w - ll) / (hh - ll))

                    # Bollinger 20
                    sma = close_w.rolling(20).mean()
                    std = close_w.rolling(20).std()
                    df_semanal['BB_Lower'] = sma - std * 2
                    df_semanal['BB_Upper'] = sma + std * 2

                    df_plot = df_semanal.dropna(subset=['Close', 'EMA20'])
                    label_tf = "Semanal"
                else:
                    df_plot  = df_ticker
                    label_tf = "Diário"
                ...
                with col1:
                    fig = plotar_grafico(df_plot, ticker,
                                        f"{row['Empresa']} [{label_tf}]",
                                        row['RSI14'], row['IS'])
                    st.pyplot(fig)
