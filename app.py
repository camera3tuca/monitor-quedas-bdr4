    ...
    with st.expander("📊 Análise de Fase — Metodologia Minervini / Weinstein", expanded=False):
        ...
        def _sma_card(col, label, val, ref, melhor_acima=True):
            ...
            <div style='background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;
                ...
                <div style='font-size:0.6rem;color:{"#16a34a" if ok else "#dc2626"};'>
                    {"▲ Preço acima" if ok else "▼ Preço abaixo"}</div>
            </div>""", unsafe_allow_html=True)
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                df_ticker_mv = df_calc.xs(ticker, axis=1, level=1).dropna()
                resultado_mv = calcular_minervini(df_ticker_mv, ticker)
            except Exception as e_mv:
                resultado_mv = {'erro': f'Erro na análise Minervini: {str(e_mv)}'}
