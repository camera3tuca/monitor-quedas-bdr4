...
def normalizar_colunas_ohlcv(df):
    """Normaliza retorno do yfinance para colunas OHLCV simples."""
    if df is None or df.empty:
        return df

    df_norm = df.copy()
    if isinstance(df_norm.columns, pd.MultiIndex):
        df_norm.columns = [c[0] if isinstance(c, tuple) else c for c in df_norm.columns]

    mapa = {str(c).lower(): c for c in df_norm.columns}
    ren = {}
    for base in ["open", "high", "low", "close", "adj close", "volume"]:
        if base in mapa:
            alvo = "Adj Close" if base == "adj close" else base.capitalize()
            ren[mapa[base]] = alvo
    if ren:
        df_norm = df_norm.rename(columns=ren)

    cols = [c for c in ["Open", "High", "Low", "Close", "Adj Close", "Volume"] if c in df_norm.columns]
    return df_norm[cols] if cols else df_norm


def baixar_dados_timeframe(ticker_b3, timeframe_label):
    """Baixa histórico com timeframe escolhido pelo usuário."""
    mapa = {
        "60 minutos": {"interval": "60m", "period": "60d"},
        "Diário": {"interval": "1d", "period": "2y"},
        "Semanal": {"interval": "1wk", "period": "5y"},
        "Mensal": {"interval": "1mo", "period": "10y"},
    }
    cfg = mapa.get(timeframe_label, mapa["Diário"])
    dados = yf.download(
        f"{ticker_b3}.SA",
        period=cfg["period"],
    ...
    return normalizar_colunas_ohlcv(dados)


def plotar_grafico_interativo(df_ticker, ticker, empresa, mostrar_candles=True):
    if df_ticker is None or df_ticker.empty:
        return None

    df = df_ticker.copy().dropna(subset=["Close"]).sort_index()
    if df.empty:
        return None
    ...
    if mostrar_candles and all(c in df.columns for c in ["Open", "High", "Low", "Close"]):
        fig.add_trace(
            go.Candlestick(
                x=df.index, open=df["Open"], high=df["High"], low=df["Low"], close=df["Close"],
                name="Candles", increasing_line_color="#2ecc71", decreasing_line_color="#e74c3c"
            ), row=1, col=1
        )
    else:
        fig.add_trace(go.Scatter(x=df.index, y=df["Close"], mode="lines", name="Close", line=dict(color="#1a1a2e", width=2)), row=1, col=1)

    if "EMA20" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["EMA20"], mode="lines", name="EMA20", line=dict(color="#2962FF", width=1.5)), row=1, col=1)
    if "EMA50" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["EMA50"], mode="lines", name="EMA50", line=dict(color="#FF6D00", width=1.5)), row=1, col=1)
    if "EMA200" in df.columns:
        fig.add_trace(go.Scatter(x=df.index, y=df["EMA200"], mode="lines", name="EMA200", line=dict(color="#00695C", width=1.6)), row=1, col=1)

    if "Volume" in df.columns:
        fig.add_trace(go.Bar(x=df.index, y=df["Volume"], name="Volume", marker_color="#90a4ae"), row=2, col=1)

    fig.update_layout(
        title=f"{ticker} - {empresa}",
...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val):
    ...
    # ── Alinhar todas as séries pelo mesmo índice de datas ──────────────────────
    # Garante que Close, EMAs, RSI etc. compartilhem exatamente as mesmas datas,
    # evitando o desalinhamento visual que ocorre quando dropna() remove linhas
    # de forma inconsistente entre colunas.
    ...
    # Remove apenas linhas onde Close OU EMA20 são NaN — mantém o máximo de dados
    ...
    datas  = df.index     # DatetimeIndex alinhado
    ...
    # EMAs — plotadas antes do Close para não sobrepor
    ...
    # Close — linha principal, mais grossa, por cima de tudo
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col1:
                    ...
                    ctf, czoom1, czoom2, ccandle = st.columns([2,1,1,1])
                    timeframe = ctf.selectbox("Timeframe", ["60 minutos", "Diário", "Semanal", "Mensal"], index=1, key=f"tf_{ticker}")
                    mostrar_candles = ccandle.toggle("Candles", value=True, key=f"candles_{ticker}")

                    dados_tf = baixar_dados_timeframe(ticker, timeframe)
                    if dados_tf is not None and not dados_tf.empty:
                        dados_tf["EMA20"] = dados_tf["Close"].ewm(span=20, adjust=False).mean()
                        dados_tf["EMA50"] = dados_tf["Close"].ewm(span=50, adjust=False).mean()
                        dados_tf["EMA200"] = dados_tf["Close"].ewm(span=200, adjust=False).mean()

                        zoom_key = f"zoom_pts_{ticker}"
                        if zoom_key not in st.session_state:
                            st.session_state[zoom_key] = min(180, len(dados_tf))

                        if czoom1.button("🔍+", help="Zoom In", key=f"zin_{ticker}"):
                            st.session_state[zoom_key] = max(20, int(st.session_state[zoom_key] * 0.7))
                        if czoom2.button("🔎-", help="Zoom Out", key=f"zout_{ticker}"):
                            st.session_state[zoom_key] = min(len(dados_tf), int(st.session_state[zoom_key] * 1.4))

                        qtd = min(st.session_state[zoom_key], len(dados_tf))
                        dados_exibir = dados_tf.tail(qtd)

                        fig_interativo = plotar_grafico_interativo(dados_exibir, ticker, row['Empresa'], mostrar_candles=mostrar_candles)
                        if fig_interativo is not None:
                            st.plotly_chart(fig_interativo, use_container_width=True)
                        else:
                            fig = plotar_grafico(df_ticker, ticker, row['Empresa'], row['RSI14'], row['IS'])
                            st.pyplot(fig)
                    else:
                        st.warning("Não foi possível carregar o timeframe selecionado. Exibindo gráfico padrão.")
                        fig = plotar_grafico(df_ticker, ticker, row['Empresa'], row['RSI14'], row['IS'])
                        st.pyplot(fig)
