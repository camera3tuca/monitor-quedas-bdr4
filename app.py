...
def buscar_dados_tradingview(ticker_us, ticker_bdr=''):
    ...
    try:
        ...
        # ── Sistema de sinais (inspirado no TradingView) ───────────────────────
        # Cada indicador vota: +1 = compra, -1 = venda, 0 = neutro
        sinais = []

        def _sinal(cond_buy, cond_sell):
            if cond_buy:   return 1
            if cond_sell:  return -1
            return 0

        # RSI
        if rsi_v:
            sinais.append(_sinal(rsi_v < 30, rsi_v > 70))
            sinais.append(_sinal(rsi_v < 50 and rsi_prev and rsi_v > rsi_prev,
                                  rsi_v > 50 and rsi_prev and rsi_v < rsi_prev))
        # MACD
        if macd_hv is not None:
            sinais.append(_sinal(macd_hv > 0, macd_hv < 0))
        if macd_v is not None and macd_sv is not None:
            sinais.append(_sinal(macd_v > macd_sv, macd_v < macd_sv))
        # Médias móveis vs preço
        if sma20: sinais.append(_sinal(preco > sma20, preco < sma20))
        if sma50: sinais.append(_sinal(preco > sma50, preco < sma50))
        if sma200:sinais.append(_sinal(preco > sma200,preco < sma200))
        if ema20: sinais.append(_sinal(preco > ema20, preco < ema20))
        if ema50: sinais.append(_sinal(preco > ema50, preco < ema50))
        # Cruzamento de médias
        if sma20 and sma50:  sinais.append(_sinal(sma20 > sma50, sma20 < sma50))
        if sma50 and sma200: sinais.append(_sinal(sma50 > sma200,sma50 < sma200))
        # Estocástico
        if stk_v: sinais.append(_sinal(stk_v < 20, stk_v > 80))
        if stk_v and std_v: sinais.append(_sinal(stk_v > std_v, stk_v < std_v))
        # CCI
        if cci_v: sinais.append(_sinal(cci_v > 100, cci_v < -100))
        # Bollinger
        if bb_u and bb_l:
            sinais.append(_sinal(preco < bb_l, preco > bb_u))

        buys    = sinais.count(1)
        sells   = sinais.count(-1)
        neutral = sinais.count(0)
        total_s = len(sinais) or 1
        rec_score = round((buys - sells) / total_s, 3)
        ...
        return {
            ...
            'mktcap': mktcap, 'eps': info.get('trailingEps'),
            'pe': _r(info.get('trailingPE'), 1), 'pb': _r(info.get('priceToBook'), 2),
            'div_yield': _r(div_y), 'setor': info.get('sector', ''),
