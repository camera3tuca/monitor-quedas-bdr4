...
def baixar_dados_grafico_ticker(ticker, periodo, intervalo):
    """Baixa dados de um ticker para o gráfico técnico com período/intervalo customizáveis."""
    try:
        return yf.download(f"{ticker}.SA", period=periodo, interval=intervalo, auto_adjust=True, progress=False, timeout=60)
    except Exception:
        return pd.DataFrame()

def calcular_indicadores_ticker(df_ticker):
    """Calcula indicadores técnicos para um único ticker (DataFrame simples)."""
...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val, tipo_grafico="Candles"):
    ...
    # Preço principal: candles ou linha
    if tipo_grafico == "Candles" and all(col in df.columns for col in ['Open', 'High', 'Low', 'Close']):
        ...
        for i, (_, row_) in enumerate(df.iterrows()):
            o, h, l, c = row_['Open'], row_['High'], row_['Low'], row_['Close']
            cor = '#2e7d32' if c >= o else '#c62828'
            ax1.vlines(i, l, h, color=cor, linewidth=0.9, alpha=0.9, zorder=4)
            corpo = max(abs(c - o), 1e-6)
            ax1.add_patch(mpatches.Rectangle((i - candle_width / 2, min(o, c)), candle_width, corpo,
                                             facecolor=cor, edgecolor=cor, alpha=0.85, zorder=5))
        ax1.plot(np.arange(len(datas)), close.values, color='#1a1a2e', linewidth=1.0, alpha=0.35, zorder=3, label='Close')
        ax1.set_xlim(-1, len(datas))
    else:
        ax1.plot(datas, close, label='Close', color='#1a1a2e', linewidth=2.2, zorder=5)

    # Ponto de fechamento mais recente destacado
    marcador_x = len(datas)-1 if tipo_grafico == "Candles" else datas[-1]
    ax1.scatter([marcador_x], [close.iloc[-1]], color='#e74c3c', s=40, zorder=6)
    ...
    if tipo_grafico == "Candles":
        tick_pos = np.linspace(0, len(datas) - 1, min(8, len(datas))).astype(int)
        tick_labels = [datas[i].strftime('%d/%m/%y') for i in tick_pos]
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                ...
                with col1:
                    ...
                    with col_cfg1:
                        timeframe = st.selectbox("Timeframe", ["60 minutos", "Diário", "Semanal"], index=1, key=f"tf_{ticker}")
                    with col_cfg2:
                        tipo_graf = st.selectbox("Tipo de gráfico", ["Candles", "Linha"], index=0, key=f"tipo_{ticker}")

                    mapa_tf = {
                        "60 minutos": {"periodo": "60d", "intervalo": "60m"},
                        "Diário": {"periodo": "1y", "intervalo": "1d"},
                        "Semanal": {"periodo": "5y", "intervalo": "1wk"},
                    }
                    cfg = mapa_tf[timeframe]
                    df_plot_bruto = baixar_dados_grafico_ticker(ticker, cfg["periodo"], cfg["intervalo"])
                    if df_plot_bruto.empty:
                        st.warning("Não foi possível carregar dados do timeframe selecionado; usando diário padrão.")
                        df_plot_bruto = baixar_dados_grafico_ticker(ticker, "1y", "1d")
                    df_plot = calcular_indicadores_ticker(df_plot_bruto)

                    if df_plot.empty:
                        st.error("Sem dados para renderizar o gráfico técnico.")
                    else:
                        fig = plotar_grafico(df_plot, ticker, row['Empresa'], row['RSI14'], row['IS'], tipo_graf)
                        st.pyplot(fig)
