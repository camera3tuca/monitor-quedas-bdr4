...
def prever_preco_ml(df_ticker, ticker, dias_previsao=5):
    """
    Treina um modelo de Regressão Linear com dados históricos do ativo
    e prevê os próximos `dias_previsao` dias.

    Features usadas: Preço de Fechamento, EMA20, RSI14.
    Target: Preço de fechamento do dia seguinte (shifted -1).

    Retorna dict com:
      - previsoes: list[float]  — preços previstos (D+1 … D+N)
      - direcao: "ALTA" | "BAIXA" | "LATERAL"
      - variacao_pct: float     — variação % do D atual ao último dia previsto
      - confianca: float        — R² do modelo no conjunto de teste (0-1)
      - ultimo_preco: float
      - erro: str | None        — mensagem de erro se falhar
    """
    try:
        ...
        # Target: preço do dia seguinte
        df = df.copy()
        df['Target'] = df['Close'].shift(-1)
        df = df.dropna()

        X = df[['Close', 'EMA20', 'RSI14']].values
        y = df['Target'].values
        ...
        # Split treino/teste (80/20, sem embaralhar — série temporal)
        ...
        # Confiança (R² no teste)
        confianca = max(0.0, float(modelo.score(X_test, y_test)))

        # --- Previsão iterativa para os próximos `dias_previsao` dias ---
        ultimo_preco  = float(df['Close'].iloc[-1])
        ultima_ema20  = float(df['EMA20'].iloc[-1])
        ultimo_rsi14  = float(df['RSI14'].iloc[-1])

        previsoes = []
        preco_atual = ultimo_preco
        ema20_atual = ultima_ema20
        rsi_atual   = ultimo_rsi14

        alpha_ema = 2 / (20 + 1)  # fator de suavização EMA20

        for _ in range(dias_previsao):
            entrada = np.array([[preco_atual, ema20_atual, rsi_atual]])
            entrada_scaled = scaler_X.transform(entrada)
            prev_scaled = modelo.predict(entrada_scaled)
            preco_previsto = float(scaler_y.inverse_transform(prev_scaled.reshape(-1, 1))[0][0])

            previsoes.append(round(preco_previsto, 2))

            # Atualiza features para o próximo passo
            delta = preco_previsto - preco_atual
            ganho = max(delta, 0)
            perda = max(-delta, 0)
            # RSI simplificado (manutenção suave)
            rsi_atual = min(max(rsi_atual + (ganho - perda) * 0.5, 0), 100)
            ema20_atual = alpha_ema * preco_previsto + (1 - alpha_ema) * ema20_atual
            preco_atual = preco_previsto

        variacao_pct = ((previsoes[-1] - ultimo_preco) / ultimo_preco) * 100
...
def renderizar_painel_ml(resultado_ml, ticker, empresa, dias_previsao=5):
    ...
    with st.expander("🤖 Previsão por Inteligência Artificial (Machine Learning)", expanded=False):
        ...
        <div style='background: linear-gradient(135deg, #1e1b4b 0%, #312e81 100%);
            ...
            <p style='margin: 0; color: #c7d2fe; font-size: 0.82rem; line-height: 1.5;'>
                🧠 <strong style='color: #a5b4fc;'>Como funciona:</strong>
                Um modelo de <strong>Regressão Linear</strong> foi treinado agora mesmo
                com os dados históricos deste ativo. Ele aprendeu a relação entre
                <em>Preço de Fechamento</em>, <em>EMA20</em> e <em>RSI14</em>
                para estimar o preço do dia seguinte — repetido por {dias} dias consecutivos.
                <br><br>
        ...
        """.format(dias=dias_previsao), unsafe_allow_html=True)
        ...
        with col_dir:
            ...
            <div style='background: linear-gradient(135deg, {bg1} 0%, {bg2} 100%);
                ...
                <div style='font-weight: 800; font-size: 1.1rem; color: {cor_txt};'>{label}</div>
                <div style='font-size: 0.8rem; color: {cor_txt}; margin-top: 0.3rem;'>
                    próximos {dias_previsao} dias
                </div>
            </div>""", unsafe_allow_html=True)

        with col_conf:
            cor_conf = "#15803d" if confianca >= 60 else "#b45309" if confianca >= 40 else "#b91c1c"
            st.markdown(f"""
            <div style='background: #f8fafc; border: 2px solid #e2e8f0;
                        padding: 1.2rem; border-radius: 10px; text-align: center;'>
                <div style='font-size: 1.8rem; font-weight: 800; color: {cor_conf};'>{confianca:.0f}%</div>
                <div style='font-size: 0.82rem; color: #64748b; margin-top: 0.3rem;'>
                    Confiança do Modelo<br><span style='font-size:0.72rem;'>(R² no conjunto de teste)</span>
                </div>
            </div>""", unsafe_allow_html=True)

        with col_var:
            sinal = "+" if variacao >= 0 else ""
            cor_var = "#15803d" if variacao > 0 else "#b91c1c" if variacao < 0 else "#78350f"
            st.markdown(f"""
            <div style='background: #f8fafc; border: 2px solid #e2e8f0;
                        padding: 1.2rem; border-radius: 10px; text-align: center;'>
                <div style='font-size: 1.8rem; font-weight: 800; color: {cor_var};'>
                    {sinal}{variacao:.2f}%
                </div>
                <div style='font-size: 0.82rem; color: #64748b; margin-top: 0.3rem;'>
                    Variação Prevista<br><span style='font-size:0.72rem;'>(D0 → D+{dias_previsao})</span>
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        # --- Gráfico de previsão ---
        dias_labels = [f"D+{i+1}" for i in range(dias_previsao)]
        todos_precos = [ult_preco] + previsoes
        todos_labels = ["Hoje"] + dias_labels
        ...
        # Área preenchida
        ax.fill_between(range(len(todos_precos)), todos_precos,
                        alpha=0.15, color=cor_linha)
        # Linha principal
        ax.plot(range(len(todos_precos)), todos_precos,
                color=cor_linha, linewidth=2.5, marker='o',
                markersize=6, markerfacecolor='white',
                markeredgecolor=cor_linha, markeredgewidth=2)

        # Ponto "Hoje" destacado
        ax.scatter([0], [ult_preco], color='#6366f1', s=100, zorder=5)

        # Linha tracejada no preço atual
        ax.axhline(ult_preco, color='#94a3b8', linestyle='--', linewidth=1, alpha=0.6)

        # Anotações dos preços
        for i, p in enumerate(todos_precos):
            ax.annotate(f'R${p:.2f}',
                        xy=(i, p),
                        xytext=(0, 10),
                        textcoords='offset points',
                        ha='center', va='bottom',
                        fontsize=7.5, color='#334155', fontweight='600')

        ax.set_xticks(range(len(todos_labels)))
        ...
        ax.set_title(f'Previsão ML — {ticker} ({empresa})',
        ...
        for i, (col, preco) in enumerate(zip(cols_prev, previsoes)):
            delta_d = preco - ult_preco
            delta_pct = (delta_d / ult_preco) * 100
            sinal = "+" if delta_pct >= 0 else ""
            cor = "green" if delta_pct >= 0 else "red"
            with col:
                st.markdown(f"""
                <div style='background:#fff; border:1px solid #e2e8f0; border-radius:8px;
                            padding:0.6rem; text-align:center;'>
                    <div style='font-size:0.72rem; color:#94a3b8; font-weight:600;'>D+{i+1}</div>
                    <div style='font-size:0.95rem; font-weight:800; color:#1e293b;'>R${preco:.2f}</div>
                    <div style='font-size:0.72rem; color:{cor};'>{sinal}{delta_pct:.1f}%</div>
                </div>""", unsafe_allow_html=True)

        # --- Legenda de confiança ---
        st.markdown("""
        <div style='margin-top:1rem; padding:0.75rem; background:#f1f5f9;
                    border-radius:8px; font-size:0.78rem; color:#64748b;'>
            📐 <strong>Interpretação da Confiança (R²):</strong>
            &nbsp; 🟢 ≥ 60% = Boa &nbsp;|&nbsp; 🟡 40-60% = Moderada &nbsp;|&nbsp;
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            resultado_ml = prever_preco_ml(
                df_calc.xs(ticker, axis=1, level=1).dropna(),
                ticker,
                dias_previsao=5
            )
