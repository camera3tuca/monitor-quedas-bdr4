...
def buscar_noticias_com_traducao(ticker_us, empresa_nome=''):
    ...
    # Palavras-chave para filtro de relevância — quanto mais específico, melhor
    # Inclui: ticker US, ticker BDR sem sufixo, e palavras do nome da empresa
    ticker_bdr_root = re.sub(r'[0-9]+$', '', ticker_us) if len(ticker_us) <= 5 else ticker_us
    palavras_chave = {
        ticker_us.lower(),           # ex: 'nvda'
        ticker_bdr_root.lower(),     # ex: 'nvd' (raramente útil, mas inofensivo)
    }
    # Adiciona palavras do nome da empresa (4+ letras, sem stopwords corporativas)
    _stopwords = {
    ...
    if empresa_nome:
        for w in empresa_nome.split():
            wc = re.sub(r'[^\w]', '', w)
            if len(wc) >= 4 and wc.lower() not in _stopwords:
                palavras_chave.add(wc.lower())

    # Remove palavras genéricas que causariam falsos positivos (ex: 'tech', 'global')
    _genericas = {'tech','technology','global','international','financial','capital',
                  'services','systems','solutions','management','digital','energy'}
    palavras_chave -= _genericas
...
@st.cache_data(ttl=300, show_spinner=False)   # cache 5min — dados quase em tempo real
def buscar_dados_tradingview(ticker_us, ticker_bdr=''):
    """
    Busca dados ao vivo do TradingView para um ticker US via tradingview-screener.
    Retorna dict com indicadores técnicos, fundamentais e recomendação.
    """
    try:
        from tradingview_screener import Query, col

        # Campos desejados — mistura técnicos + fundamentais + recomendação
        campos = [
        ...
        # Tenta com exchange explícita (NASDAQ, NYSE, AMEX)
        df = None
        for exchange in ['NASDAQ', 'NYSE', 'AMEX', '']:
            try:
                ticker_query = f"{exchange}:{ticker_us}" if exchange else ticker_us
                resultado = (
                    Query()
                    .select(*campos)
        ...
        if df is None or len(df) == 0:
            return {'erro': f'Ticker {ticker_us} não encontrado no TradingView.'}

        row = df.iloc[0].to_dict()

        # Interpreta a recomendação (-1 a +1 → texto)
        rec_val = row.get('Recommend.All', None)
        def _rec_label(v):
            if v is None:
                return 'N/A', '#94a3b8'
            v = float(v)
        ...
        rec_label, rec_cor = _rec_label(rec_val)

        # Conta sinais
        buys    = int(row.get('buy_signals_count',     0) or 0)
        sells   = int(row.get('sell_signals_count',    0) or 0)
        neutral = int(row.get('neutral_signals_count', 0) or 0)
        total_s = buys + sells + neutral or 1
        ...
        return {
            ...
            'macd_hist'     : _f('MACD.hist'),
            'stoch_k'       : _f('Stoch.K', 1),
            'stoch_d'       : _f('Stoch.D', 1),
            'cci'           : _f('CCI20', 1),
            'adx'           : _f('ADX', 1),
            'bb_upper'      : _f('BB.upper'),
            'bb_lower'      : _f('BB.lower'),
            'bb_basis'      : _f('BB.basis'),
            'vol_rel'       : _f('relative_volume_10d_calc', 2),
            'vol_avg10'     : _f('average_volume_10d_calc', 0),
            'rec_val'       : rec_val,
            'rec_label'     : rec_label,
            'rec_cor'       : rec_cor,
            'rec_ma'        : _f('Recommend.MA'),
            'rec_outros'    : _f('Recommend.Other'),
            'buys'          : buys,
            'sells'         : sells,
            'neutral'       : neutral,
            'total_sinais'  : total_s,
            'mktcap'        : _f('market_cap_basic', 0),
            'eps'           : _f('earnings_per_share_basic_ttm'),
            'pe'            : _f('price_earnings_ttm', 1),
            'pb'            : _f('price_book_ratio', 2),
            'div_yield'     : _f('dividends_yield_current', 2),
            'setor'         : row.get('sector', ''),
            'industria'     : row.get('industry', ''),
            'atr'           : _f('ATR'),
            'volatilidade'  : _f('Volatility.D', 2),
            'max_52s'       : _f('price_52_week_high'),
            'min_52s'       : _f('price_52_week_low'),
        }

    except ImportError:
        return {'erro': 'tradingview-screener não instalado. Adicione ao requirements.txt.'}
    except Exception as e:
        return {'erro': f'Erro ao buscar dados do TradingView: {str(e)}'}
        ...
        <div style='background:linear-gradient(135deg,#131722 0%,#1e2a3a 100%);
            ...
            <div>
                <div style='color:#2962ff;font-weight:800;font-size:1rem;'>
                    TradingView Screener — {ticker_us} ({empresa})</div>
                <div style='color:#787b86;font-size:0.75rem;'>
                    Dados via API oficial TradingView · github.com/shner-elmo/TradingView-Screener
