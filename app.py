...
def buscar_dados_tradingview(ticker_us, ticker_bdr=''):
    """
    Busca dados técnicos e fundamentais em cascata:
      1. tradingview-screener (se instalado)
      2. yfinance — fallback universal, sempre disponível no Streamlit Cloud
    """
    def _rec_label(v):
    ...
    def _fv(d, k, dec=2):
        ...
        try:
            f = float(v)
            return None if str(f) in ('nan','inf','-inf') else round(f, dec)
        except Exception:
            return None

    def _tv_to_result(row_dict):
        rv = row_dict.get('Recommend.All')
        rl, rc = _rec_label(rv)
        b = int(row_dict.get('buy_signals_count',     0) or 0)
        s = int(row_dict.get('sell_signals_count',    0) or 0)
        n = int(row_dict.get('neutral_signals_count', 0) or 0)
        tot = b + s + n or 1
        return {
            'erro': None, 'fonte': 'TradingView', 'ticker': ticker_us,
            'close':       _fv(row_dict,'close'),
            'open':        _fv(row_dict,'open'),
            'high':        _fv(row_dict,'high'),
            'low':         _fv(row_dict,'low'),
            'volume':      _fv(row_dict,'volume',0),
            'change_pct':  _fv(row_dict,'change'),
            'change_abs':  _fv(row_dict,'change_abs'),
            'sma20':       _fv(row_dict,'SMA20'),
            'sma50':       _fv(row_dict,'SMA50'),
            'sma200':      _fv(row_dict,'SMA200'),
            'ema20':       _fv(row_dict,'EMA20'),
            'ema50':       _fv(row_dict,'EMA50'),
            'rsi':         _fv(row_dict,'RSI',1),
            'rsi_prev':    _fv(row_dict,'RSI[1]',1),
            'macd':        _fv(row_dict,'MACD.macd'),
            'macd_signal': _fv(row_dict,'MACD.signal'),
            'macd_hist':   _fv(row_dict,'MACD.hist'),
            'stoch_k':     _fv(row_dict,'Stoch.K',1),
            'stoch_d':     _fv(row_dict,'Stoch.D',1),
            'cci':         _fv(row_dict,'CCI20',1),
            'adx':         _fv(row_dict,'ADX',1),
            'bb_upper':    _fv(row_dict,'BB.upper'),
            'bb_lower':    _fv(row_dict,'BB.lower'),
            'bb_basis':    _fv(row_dict,'BB.basis'),
            'vol_rel':     _fv(row_dict,'relative_volume_10d_calc',2),
            'vol_avg10':   _fv(row_dict,'average_volume_10d_calc',0),
            'rec_val': rv, 'rec_label': rl, 'rec_cor': rc,
            'rec_ma':      _fv(row_dict,'Recommend.MA'),
            'rec_outros':  _fv(row_dict,'Recommend.Other'),
            'buys': b, 'sells': s, 'neutral': n, 'total_sinais': tot,
            'mktcap':      _fv(row_dict,'market_cap_basic',0),
            'eps':         _fv(row_dict,'earnings_per_share_basic_ttm'),
            'pe':          _fv(row_dict,'price_earnings_ttm',1),
            'pb':          _fv(row_dict,'price_book_ratio',2),
            'div_yield':   _fv(row_dict,'dividends_yield_current',2),
            'setor':       row_dict.get('sector','') or '',
            'industria':   row_dict.get('industry','') or '',
            'atr':         _fv(row_dict,'ATR'),
            'volatilidade':_fv(row_dict,'Volatility.D',2),
            'max_52s':     _fv(row_dict,'price_52_week_high'),
            'min_52s':     _fv(row_dict,'price_52_week_low'),
        }

    # ── Estratégia 1: TradingView Screener (opcional) ────────────────────────
    try:
        ...
        for where_clause in [
            ...
            try:
                _, df_tv = (Query().select(*campos)
                            .where(where_clause)
                            .set_markets('america')
                            .get_scanner_data())
                if df_tv is not None and len(df_tv) > 0:
                    row = df_tv.iloc[0].to_dict()
                    if row.get('close') and float(row.get('close', 0) or 0) > 0:
                        return _tv_to_result(row)
            except Exception:
                continue
    except ImportError:
        pass

    # ── Estratégia 2: yfinance (sempre disponível) ────────────────────────────
    try:
        t    = yf.Ticker(ticker_us)
        hist = t.history(period='1y', interval='1d', auto_adjust=True)
        if hist.empty:
            return {'erro': f'Ticker {ticker_us} não encontrado.'}
        ...
        def _s(s, n): return round(float(s.rolling(n).mean().iloc[-1]),2) if len(s)>=n else None
        def _e(s, n): return round(float(s.ewm(span=n).mean().iloc[-1]),2) if len(s)>=n else None
        def _r(v, d=2): return round(v,d) if v is not None else None

        sma20=_s(close,20); sma50=_s(close,50); sma200=_s(close,200)
        ema20=_e(close,20); ema50=_e(close,50)

        delta=close.diff(); g=delta.clip(lower=0).rolling(14).mean()
        lo=(-delta.clip(upper=0)).rolling(14).mean()
        rsi_v=_r(float((100-100/(1+g/lo.replace(0,float('nan')))).iloc[-1]),1) if len(close)>=15 else None

        ema12=close.ewm(span=12).mean(); ema26=close.ewm(span=26).mean()
        ml=ema12-ema26; ms=ml.ewm(span=9).mean(); mh=ml-ms
        macd_v  =_r(float(ml.iloc[-1]),4) if len(close)>=26 else None
        macd_sv =_r(float(ms.iloc[-1]),4) if len(close)>=26 else None
        macd_hv =_r(float(mh.iloc[-1]),4) if len(close)>=26 else None

        sb=close.rolling(20).mean(); stdb=close.rolling(20).std()
        bb_u=_r(float((sb+stdb*2).iloc[-1])) if len(close)>=20 else None
        bb_l=_r(float((sb-stdb*2).iloc[-1])) if len(close)>=20 else None
        bb_b=_r(float(sb.iloc[-1]))           if len(close)>=20 else None

        l14=low_s.rolling(14).min(); h14=high.rolling(14).max()
        stk=_r(float((100*(close-l14)/(h14-l14+1e-9)).iloc[-1]),1) if len(close)>=14 else None

        tr=pd.concat([high-low_s,(high-close.shift(1)).abs(),(low_s-close.shift(1)).abs()],axis=1).max(axis=1)
        atr_v=_r(float(tr.rolling(14).mean().iloc[-1])) if len(tr)>=14 else None

        va=float(vol.rolling(10).mean().iloc[-1]) if len(vol)>=10 else None
        vr=_r(float(vol.iloc[-1]/va),2) if va and va>0 else None

        pc=float(close.iloc[-1]); pp=float(close.iloc[-2]) if len(close)>=2 else pc
        var=(pc-pp)/pp*100 if pp else 0.0
        ...
        if sma20 and sma50 and sma200:
            if pc>sma20>sma50>sma200: sc+=3
            elif pc<sma20<sma50<sma200: sv+=3
        elif sma20 and sma50:
            if pc>sma20>sma50: sc+=2
            elif pc<sma20<sma50: sv+=2
        if macd_hv:
            if macd_hv>0: sc+=1
            else: sv+=1
        if stk:
        ...
        total_sinais=sc+sv+1
        rec_score=round((sc-sv)/total_sinais,2)
        rl,rc=_rec_label(rec_score)

        max52=_r(float(high.iloc[-252:].max())) if len(high)>=30 else None
        min52=_r(float(low_s.iloc[-252:].min())) if len(low_s)>=30 else None
        dy=info.get('dividendYield')

        return {
            'erro':None,'fonte':'yfinance','ticker':ticker_us,
            'close':_r(pc),'open':_r(float(hist['Open'].iloc[-1])),
            'high':_r(float(high.iloc[-1])),'low':_r(float(low_s.iloc[-1])),
            'volume':float(vol.iloc[-1]),'change_pct':_r(var),'change_abs':_r(pc-pp),
            'sma20':sma20,'sma50':sma50,'sma200':sma200,'ema20':ema20,'ema50':ema50,
            'rsi':rsi_v,'rsi_prev':None,
            'macd':macd_v,'macd_signal':macd_sv,'macd_hist':macd_hv,
            'stoch_k':stk,'stoch_d':None,'cci':None,'adx':None,
            'bb_upper':bb_u,'bb_lower':bb_l,'bb_basis':bb_b,
            'vol_rel':vr,'vol_avg10':_r(va,0) if va else None,
            'rec_val':rec_score,'rec_label':rl,'rec_cor':rc,
            'rec_ma':None,'rec_outros':None,
            'buys':sc,'sells':sv,'neutral':1,'total_sinais':total_sinais,
            'mktcap':info.get('marketCap'),
            'eps':info.get('trailingEps'),
            'pe':_r(float(info['trailingPE']),1) if info.get('trailingPE') else None,
            'pb':_r(float(info['priceToBook']),2) if info.get('priceToBook') else None,
            'div_yield':_r(float(dy)*100,2) if dy else None,
            'setor':info.get('sector','') or '','industria':info.get('industry','') or '',
            'atr':atr_v,'volatilidade':None,'max_52s':max52,'min_52s':min52,
        }
    except Exception as e:
        return {'erro': f'Erro ao buscar {ticker_us}: {str(e)}'}
...
def renderizar_painel_tradingview(dados, ticker_us, empresa, peers=None):
    ...
    with st.expander("📡 TradingView — Dados ao Vivo & Recomendação", expanded=False):
        ...
        vol_cor = '#26a69a' if vol_rel >= 1.5 else '#ef5350' if vol_rel < 0.7 else '#787b86'
        vol_lbl = 'Volume alto' if vol_rel >= 1.5 else 'Volume baixo' if vol_rel < 0.7 else 'Volume normal'
        with c4:
            st.markdown(f"""
            <div style='background:#131722;border:1px solid #2a2e39;
                        border-radius:10px;padding:0.85rem;text-align:center;min-height:120px;
        ...
        # ── Linha 2: Indicadores técnicos ─────────────────────────────────────
        st.markdown("**📊 Indicadores Técnicos (TradingView — tempo real):**")

        def _ind_card(col_st, nome, valor, referencia=None, unidade='', inv=False,
                      low_good=None, high_bad=None, fmt='.1f'):
            ...
            # Determina cor
            cor = '#787b86'
            if low_good is not None and high_bad is not None:
                if valor < low_good:     cor = '#26a69a'
                elif valor > high_bad:   cor = '#ef5350'
            elif referencia is not None:
                cor = '#26a69a' if (valor > referencia) != inv else '#ef5350'
            col_st.markdown(f"""
            <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
                        padding:0.55rem;text-align:center;'>
        ...
        cols_ind = st.columns(8)
        _ind_card(cols_ind[0], 'RSI 14',    dados.get('rsi'),      50,   low_good=30,  high_bad=70)
        _ind_card(cols_ind[1], 'Stoch K',   dados.get('stoch_k'),  50,   low_good=20,  high_bad=80)
        _ind_card(cols_ind[2], 'CCI 20',    dados.get('cci'),       0,   low_good=-100,high_bad=100)
        _ind_card(cols_ind[3], 'ADX',       dados.get('adx'),      25,   referencia=25)
        _ind_card(cols_ind[4], 'MACD',      dados.get('macd'),      0,   referencia=0, fmt='.3f')
        _ind_card(cols_ind[5], 'MACD Hist', dados.get('macd_hist'), 0,   referencia=0, fmt='.3f')
        _ind_card(cols_ind[6], 'ATR',       dados.get('atr'),      None)
        _ind_card(cols_ind[7], 'Volat. D',  dados.get('volatilidade'), None, unidade='%')
        ...
        for col_st, (nome, val) in zip(cols_ma, smas):
            if val and close:
                ...
                lbl   = '▲ acima' if acima else '▼ abaixo'
                col_st.markdown(f"""
                <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
                    ...
                    <div style='font-size:0.95rem;font-weight:800;color:{cor};'>
                        ${val:.2f}</div>
                    <div style='font-size:0.6rem;color:{cor};'>{lbl}</div>
                </div>""", unsafe_allow_html=True)
            else:
                col_st.markdown(f"""
                <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
        ...
        with col_bb:
            ...
            <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
                ...
                <div style='font-size:0.78rem;color:#d1d4dc;'>
                    Superior: <strong>${bb_u:.2f if bb_u else 'N/A'}</strong></div>
                <div style='font-size:0.78rem;color:#d1d4dc;'>
                    Basis: <strong>${bb_b:.2f if bb_b else 'N/A'}</strong></div>
                <div style='font-size:0.78rem;color:#d1d4dc;'>
                    Inferior: <strong>${bb_l:.2f if bb_l else 'N/A'}</strong></div>
        ...
        with col_52:
            ...
            <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
                ...
                <div style='font-size:0.78rem;color:#d1d4dc;'>
                    Máx: <strong style='color:#ef5350;'>${max52:.2f if max52 else 'N/A'}</strong>
                    &nbsp;·&nbsp;
                    Mín: <strong style='color:#26a69a;'>${min52:.2f if min52 else 'N/A'}</strong>
                </div>
                {'<div style="background:#2a2e39;border-radius:99px;height:6px;margin:0.4rem 0;"><div style="background:#2962ff;width:' + f"{pct_range:.0f}" + '%;height:6px;border-radius:99px;"></div></div>' if pct_range is not None else ''}
                <div style='font-size:0.7rem;color:#787b86;'>
                    {'Posição: ' + f"{pct_range:.0f}% do range · {dist_max:.1f}% abaixo da máx" if pct_range is not None else 'N/A'}
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        with col_fund:
            ...
            <div style='background:#131722;border:1px solid #2a2e39;border-radius:8px;
                ...
                <div style='display:grid;grid-template-columns:1fr 1fr;gap:0.2rem;
                    ...
                    <span>P/E:</span><strong>{pe if pe else 'N/A'}</strong>
                    <span>P/B:</span><strong>{pb if pb else 'N/A'}</strong>
                    <span>EPS (TTM):</span><strong>{'${:.2f}'.format(eps) if eps else 'N/A'}</strong>
                    <span>Div. Yield:</span><strong>{'{:.2f}%'.format(div) if div else 'N/A'}</strong>
                </div>
