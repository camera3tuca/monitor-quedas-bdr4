...
def renderizar_painel_ml(resultado_ml, ticker, empresa, dias_previsao=5):
    ...
    with st.expander("🤖 Previsão por Inteligência Artificial — Ensemble de Modelos", expanded=False):
        ...
        # --- Ranking dos modelos — grid responsivo 2 linhas (evita 5 colunas espremidas) ---
        if ranking:
            st.markdown("**🏆 Desempenho dos Modelos no Conjunto de Teste:**")
            medalhas    = ['🥇','🥈','🥉','4°','5°']
            itens_html  = ""
            for i, mod_info in enumerate(ranking):
                ...
                rmse_disp = mod_info["rmse"]
                nome_curto = nomes_pt.get(mod_info["nome"], mod_info["nome"])
                barra_r2  = min(int(r2_disp * 2), 100)   # escala: 50% R² = barra cheia
                cor_bar_r = "#22c55e" if i == 0 else "#94a3b8"
                itens_html += f"""
                <div style='background:{bg_rank};border:{borda_r};border-radius:10px;
                            padding:0.7rem 0.8rem;flex:1 1 160px;min-width:140px;'>
                    <div style='display:flex;align-items:center;gap:0.4rem;margin-bottom:0.4rem;'>
                        <span style='font-size:1.1rem;'>{medalhas[i]}</span>
                        <span style='font-size:0.75rem;font-weight:700;color:{cor_rank};
                                     white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>
                            {icones_modelo.get(mod_info["nome"],"")} {nome_curto}</span>
                    </div>
                    <div style='font-size:1.3rem;font-weight:900;color:{cor_rank};
                                line-height:1;margin-bottom:0.25rem;'>
                        {r2_disp:.1f}%</div>
                    <div style='background:#e2e8f0;border-radius:99px;height:5px;margin-bottom:0.3rem;'>
                        <div style='background:{cor_bar_r};width:{barra_r2}%;height:5px;border-radius:99px;'></div>
                    </div>
                    <div style='font-size:0.65rem;color:#94a3b8;'>
                        R² &nbsp;·&nbsp; RMSE: {rmse_disp:.5f}</div>
                </div>"""
...
class _QNetwork:
    """Rede neural MLP simples implementada em NumPy puro (sem Keras/TF)."""
    def __init__(self, state_size, action_size, lr=0.001):
        ...
        self.b2 = np.zeros(32)
        self.W3 = np.random.randn(32, action_size) * np.sqrt(2 / 32)
        self.b3 = np.zeros(action_size)

    def predict(self, x):
        x = np.asarray(x, dtype=np.float32).flatten()
        self.z1 = _relu(x @ self.W1 + self.b1)
        self.z2 = _relu(self.z1 @ self.W2 + self.b2)
    ...
    def train(self, x, y_target):
        ...
        # Loss = MSE
        d_out = 2 * (out - y_target) / len(y_target)
        # Backprop W3/b3
...
class _RLAgent:
    """Agente de Q-Learning com Experience Replay e política ε-greedy."""
...
def _get_state_rl(prices, t, window):
    """Estado = diferenças de preços normalizadas por sigmoid (igual ao notebook)."""
    d = t - window + 1
    block = prices[max(0, d):t + 1]
    if len(block) < window:
        block = np.concatenate([np.full(window - len(block), block[0]), block])
    res = [_sigmoid(float(block[i + 1]) - float(block[i]))
           for i in range(window - 1)]
    return np.array(res, dtype=np.float32)


def executar_agente_rl(df_ticker, ticker, episodes=8, window_size=5, max_steps=None):
    """
    Treina o agente RL nos dados históricos (80%) e avalia no conjunto de teste (20%).
    Retorna dict com:
      - acoes_treino / acoes_teste : listas de (dia_idx, acao, preco)
      - lucro_treino / lucro_teste : float
      - precos_treino / precos_teste: np.array
      - historico_lucro_episodios  : list[float]
      - episodios                  : int
      - erro                       : str | None
      - recomendacao               : 'COMPRAR' | 'VENDER' | 'AGUARDAR'
      - ultima_acao_desc           : str
    """
    try:
        df = df_ticker.copy().sort_index()
        ...
        n_passos_max = max_steps or len(train_data)
        ...
        for ep in range(episodes):
            state = _get_state_rl(train_data, 0, window_size + 1)
            total_profit = 0.0
            agent.inventory = []
            l = min(len(train_data) - 1, n_passos_max)

            for t in range(l):
                action = agent.act(state)
                next_state = _get_state_rl(train_data, t + 1, window_size + 1)
                reward = 0.0
                ...
                if action == 1:   # buy
                    ...
                    reward = max(train_data[t] - bought, 0)
                    total_profit += train_data[t] - bought
        ...
        # ── AVALIAÇÃO (test set) ──────────────────────────────────────────────────
        state = _get_state_rl(test_data, 0, window_size + 1)
        total_profit_test = 0.0
        agent.inventory = []
        compras_test, vendas_test = [], []

        for t in range(len(test_data) - 1):
            action = agent.act(state, is_eval=True)
            next_state = _get_state_rl(test_data, t + 1, window_size + 1)

            if action == 1:
                agent.inventory.append(test_data[t])
                compras_test.append((t, test_data[t]))
            elif action == 2 and agent.inventory:
                bought = agent.inventory.pop(0)
                lucro_op = test_data[t] - bought
                total_profit_test += lucro_op
                vendas_test.append((t, test_data[t], lucro_op))

            state = next_state

        # Última ação do agente no ponto mais recente
        ultimo_estado = _get_state_rl(prices, len(prices) - 1, window_size + 1)
        ultima_q      = agent.model.predict(ultimo_estado)
        ultima_acao   = int(np.argmax(ultima_q))
        acoes_desc    = {0: 'AGUARDAR', 1: 'COMPRAR', 2: 'VENDER'}
        recomendacao  = acoes_desc[ultima_acao]

        return {
            'erro'                      : None,
            'precos_treino'             : train_data,
            'precos_teste'              : test_data,
...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val,
    ...
    # ── Reamostrar conforme timeframe ────────────────────────────────────────────
    # Os dados base sempre chegam em frequência diária (yfinance 1y/1d).
    # Para 60min precisaríamos rebaixar, mas como o yfinance baixa diário,
    # simulamos "Horário" mostrando apenas os últimos 30 dias com mais granularidade.
    # Para Semanal e Mensal, reamostrar via resample.
    ...
    df_full = df_ticker[colunas_presentes].copy()
    df_full = df_full.dropna(subset=['Close', 'EMA20'])
    df_full = df_full.sort_index()

    # Reamostrar OHLCV para o timeframe escolhido
    tf_label = timeframe
    if timeframe == 'Semanal':
        ohlcv_cols = {c: ('first' if c == 'Open' else
                          'max'   if c == 'High'  else
                          'min'   if c == 'Low'   else
                          'last'  if c == 'Close' else
                          'sum'   if c == 'Volume' else
                          'last') for c in colunas_presentes}
        df = df_full.resample('W').agg(ohlcv_cols).dropna(subset=['Close'])
        ...
        ohlcv_cols = {c: ('first' if c == 'Open' else
                          ...
                          'sum'   if c == 'Volume' else
                          'last') for c in colunas_presentes}
        df = df_full.resample('ME').agg(ohlcv_cols).dropna(subset=['Close'])
    elif timeframe == 'Horário (60min)':
        # Com dados diários, mostramos os últimos 60 dias como proxy de "curto prazo"
        df = df_full.iloc[-60:].copy()
    ...
    # ── Eixo X com datas legíveis (adaptado ao timeframe) ───────────────────────
    ax3 = axes[2]
    if timeframe == 'Mensal':
        ...
        ax3.xaxis.set_major_locator(mdates.WeekdayLocator(byweekday=0, interval=4))
    elif timeframe == 'Horário (60min)':
    ...
    plt.setp(ax3.xaxis.get_majorticklabels(),
             rotation=40, ha='right', fontsize=9, fontweight='600',
             color='#334155')
    ax3.tick_params(axis='x', which='major', pad=6, length=4, color='#94a3b8')
    ax3.set_xlabel('Data', fontsize=10, color='#475569', labelpad=8)

    # Adiciona linha de grade vertical no eixo X para facilitar leitura
    for ax in axes:
        ax.xaxis.set_tick_params(which='major', gridOn=True)
        ax.grid(axis='x', alpha=0.10, color='#94a3b8', linestyle='-')

    plt.tight_layout(rect=[0, 0, 1, 1])
    fig.subplots_adjust(bottom=0.13)   # espaço extra embaixo para os rótulos
    return fig
...
if 'oportunidades' in st.session_state and 'df_calc' in st.session_state:
    ...
    if not df_res.empty:
        ...
        if evento.selection and evento.selection.rows:
            ...
            try:
                df_ticker_rl = df_calc.xs(ticker, axis=1, level=1).dropna()
                resultado_rl = executar_agente_rl(df_ticker_rl, ticker, episodes=8, window_size=5)
            except Exception as e_rl:
                resultado_rl = {'erro': f'Não foi possível executar o agente RL: {str(e_rl)}'}
