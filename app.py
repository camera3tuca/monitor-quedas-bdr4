...
def calcular_indicadores(df):
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            # ÍNDICE DE FORÇA DE ELDER (EFI)
            # Fórmula: (Fechamento atual - Fechamento anterior) × Volume
            # EFI bruto suavizado por EMA de 13 períodos (tendência) e 2 períodos (curto prazo)
            volume = df_calc[('Volume', ticker)]
            efi_bruto = close.diff() * volume
            df_calc[('EFI13', ticker)] = efi_bruto.ewm(span=13, adjust=False).mean()  # tendência
            df_calc[('EFI2',  ticker)] = efi_bruto.ewm(span=2,  adjust=False).mean()  # curto prazo
        except: continue
...
def gerar_sinal(row_ticker, df_ticker):
    ...
    try:
        ...
        efi13 = row_ticker.get('EFI13')
        efi2  = row_ticker.get('EFI2')
        ...
        # ÍNDICE DE FORÇA DE ELDER (EFI)
        # Sinal principal: EFI13 cruzando de negativo para positivo enquanto preço caiu
        # = pressão compradora se fortalecendo = divergência de alta = possível reversão
        if pd.notna(efi13) and pd.notna(efi2):
            efi13_hist = df_ticker['EFI13'].dropna() if 'EFI13' in df_ticker.columns else None
            efi2_hist  = df_ticker['EFI2'].dropna()  if 'EFI2'  in df_ticker.columns else None

            # Sinal 1: EFI13 negativo mas subindo (pressão vendedora diminuindo)
            if efi13 < 0 and efi13_hist is not None and len(efi13_hist) >= 3:
                efi13_anterior = efi13_hist.iloc[-2]
                if efi13 > efi13_anterior:
                    sinais.append("EFI Virando")
                    explicacoes.append(
                        f"⚡ EFI Elder em {efi13:,.0f}: negativo mas subindo — "
                        "vendedores perdendo força, possível reversão de alta se confirmar"
                    )
                    score += 2

            # Sinal 2: EFI2 (curto prazo) cruzou zero para positivo = entrada de compradores agora
            if efi2 > 0 and efi13 < 0:
                sinais.append("EFI Impulso Alta")
                explicacoes.append(
                    f"🔋 EFI2 positivo ({efi2:,.0f}) com EFI13 ainda negativo: "
                    "compradores entraram no curto prazo — monitorar confirmação"
                )
                score += 1

            # Sinal 3: ambos positivos = força compradora confirmada em duas escalas
            if efi2 > 0 and efi13 > 0:
                sinais.append("EFI Força Alta")
                explicacoes.append(
                    "💪 EFI2 e EFI13 ambos positivos: "
                    "compradores dominam tanto no curto quanto no médio prazo"
                )
                score += 1
...
def plotar_grafico(df_ticker, ticker, empresa, rsi, is_val):
    ...
    # ÍNDICE DE FORÇA DE ELDER (EFI)
    ax4 = axes[3]
    if 'EFI13' in df_ticker.columns and 'EFI2' in df_ticker.columns:
        efi13_vals = df_ticker['EFI13'].dropna()
        efi2_vals  = df_ticker['EFI2'].dropna()
        idx_comum  = efi13_vals.index.intersection(efi2_vals.index)

        if len(idx_comum) > 0:
            e13 = efi13_vals.loc[idx_comum]
            e2  = efi2_vals.loc[idx_comum]

            # Histograma colorido: verde quando positivo, vermelho quando negativo
            cores_hist = ['#26a69a' if v >= 0 else '#ef5350' for v in e13]
            ax4.bar(idx_comum, e13, color=cores_hist, alpha=0.55, width=1.2, label='EFI13 (tendência)')

            # Linha EFI2 (curto prazo) por cima
            ax4.plot(idx_comum, e2, color='#FFA726', linewidth=1.2,
                     linestyle='-', label='EFI2 (curto prazo)', alpha=0.9)

            # Linha zero
            ax4.axhline(0, color='#90A4AE', linewidth=1.2, linestyle='--', alpha=0.8)

            # Sombreamento: zona positiva (compradores) vs negativa (vendedores)
            ax4.fill_between(idx_comum, e13, 0,
                             where=(e13 >= 0), alpha=0.12, color='#26a69a', interpolate=True)
            ax4.fill_between(idx_comum, e13, 0,
                             where=(e13 < 0),  alpha=0.12, color='#ef5350', interpolate=True)

            ax4.legend(loc='upper left', fontsize=7, framealpha=0.85)

    ax4.set_ylabel('EFI Elder', fontsize=9)
    ax4.grid(True, alpha=0.2)
    ax4.set_xlabel('Data', fontsize=9)
...
with st.expander("📚 Guia dos Indicadores - Entenda os Sinais", expanded=False):
    ...
    **O que é:** Mostram a direção da tendência.
    ...
    ### ⚡ Índice de Força de Elder (EFI)
    **O que é:** Criado pelo trader e psiquiatra Alexander Elder, mede a **força real** por trás de cada movimento de preço combinando três fatores simultaneamente: **direção** (para onde o preço foi), **extensão** (o quanto subiu ou caiu) e **volume** (quantas pessoas participaram).

    **Fórmula:** `(Fechamento de hoje − Fechamento de ontem) × Volume`
    Depois suavizado por uma EMA de 13 períodos (tendência) ou 2 períodos (curto prazo).

    **Como interpretar o histograma:**
    - 🟢 **EFI acima do zero**: compradores no controle — força de alta predomina
    - 🔴 **EFI abaixo do zero**: vendedores no controle — força de baixa predomina
    - 📈 **EFI negativo mas subindo**: vendedores perdendo força → possível reversão de alta
    - 📉 **EFI positivo mas caindo**: compradores perdendo fôlego → possível reversão de baixa

    **Sinais que o monitor detecta:**
    - 🔋 **EFI Virando** — EFI13 negativo e subindo: pressão vendedora diminuindo. Favorável para reversão.
    - 💡 **EFI Impulso Alta** — EFI2 (curto prazo) cruzou o zero para positivo, mas EFI13 ainda negativo: compradores entrando agora, aguardar confirmação.
    - 💪 **EFI Força Alta** — EFI2 e EFI13 ambos positivos: compradores dominam nas duas escalas de tempo.

    **Dica de Elder:** use o EFI13 para identificar a tendência e o EFI2 para cronometrar a entrada. O sinal mais poderoso é quando o preço cai mas o EFI sobe — isso é **divergência de alta** e indica que os vendedores estão se esgotando.
    ...
    5. **Observe** o EFI: se negativo mas subindo, vendedores perdendo força 💪
    6. **Entre** quando o EFI2 cruzar o zero e o EFI13 estiver subindo! 🚀
    """)
