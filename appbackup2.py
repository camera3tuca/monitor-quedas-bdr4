...
def renderizar_painel_ml(resultado_ml, ticker, empresa, dias_previsao=5):
    ...
    with st.expander("🤖 Previsão por Inteligência Artificial — Ensemble de Modelos", expanded=False):
        ...
        with col_conf:
            ...
            <div style='background:#f8fafc;border:2px solid #e2e8f0;padding:1.2rem;
                ...
                <div style='font-size:0.8rem;color:#64748b;margin-top:0.2rem;'>
                    Confiança ({nivel})<br>
                    <span style='font-size:0.7rem;'>{icone_melhor} {nomes_pt.get(melhor_modelo,melhor_modelo)}</span>
                </div>
            </div>""", unsafe_allow_html=True)
        ...
        # --- Ranking dos modelos ---
        if ranking:
            st.markdown("**🏆 Ranking dos Modelos (R² no conjunto de teste):**")
            rank_cols = st.columns(len(ranking))
            medalhas = ['🥇','🥈','🥉','4️⃣','5️⃣']
            for i, (rc, mod_info) in enumerate(zip(rank_cols, ranking)):
                cor_rank = "#15803d" if i == 0 else "#64748b"
                borda    = "2px solid #22c55e" if i == 0 else "1px solid #e2e8f0"
                bg_rank  = "#f0fdf4" if i == 0 else "#f8fafc"
                with rc:
                    st.markdown(f"""
                    <div style='background:{bg_rank};border:{borda};border-radius:8px;
                                padding:0.55rem 0.3rem;text-align:center;'>
                        <div style='font-size:1.1rem;'>{medalhas[i]}</div>
                        <div style='font-size:0.65rem;font-weight:700;color:{cor_rank};
                                    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>
                            {icones_modelo.get(mod_info["nome"],"")}{nomes_pt.get(mod_info["nome"],mod_info["nome"])}</div>
                        <div style='font-size:0.85rem;font-weight:800;color:{cor_rank};'>
                            {mod_info["r2"]*100:.1f}%</div>
                        <div style='font-size:0.6rem;color:#94a3b8;'>R²</div>
                    </div>""", unsafe_allow_html=True)
